...
**P5 色彩**
  ...
  • 渐变、透明度、叠加模式
  • **注意区分文字颜色和背景颜色**：白色/浅色文字在深色背景上可能视觉上显得偏暗，但实际文字颜色仍然是 #FFFFFF 或接近白色。请仔细辨别文字本身的颜色值，不要被背景色干扰判断
  • 如果两图的文字颜色都是白色或近白色（如 #FFFFFF、#FAFAFA、#F0F0F0），不要报告为颜色差异
  • 只有色值差距明显（HEX 值相差较大，视觉上肉眼可辨）时才标记为色彩问题
