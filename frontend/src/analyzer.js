...
适配分析提示：
${gameRatio === designRatio ? '• 实机截图与设计分辨率宽高比一致，可直接逐像素比较位置/间距/大小' :
  gameRatio > designRatio ? `• 实机截图比设计分辨率更宽（${gameRatio} > ${designRatio}），高度方向 1:1 缩放，宽度方向有额外拉伸区域。靠左右边缘的元素水平位置可能因拉伸而偏移，这属于正常适配。但元素自身的尺寸（宽高、字号、图标大小）不应因适配而改变，有差异仍需报告` :
  `• 实机截图比设计分辨率更窄（${gameRatio} < ${designRatio}），整体缩小后高度方向拉伸。比较间距和尺寸时需除以缩放因子 ${gameScale.toFixed(4)} 换算到设计分辨率坐标系，换算后的尺寸/字号仍应与设计稿一致`}
...
所有颜色描述统一使用 **#HEX (中文色名)** 格式，十六进制色值以实际图像取色为准，括号内附上通俗中文色名作为语义描述，例如：#F26522 (橙红)、#FEEEED (淡粉)、#7A1723 (暗红)、#FFFFFF (白色)、#333333 (深灰)。
${adaptationInfo}
...
**关键区分：适配影响的是布局位置，不影响组件自身的尺寸和字号。**
- 适配可能改变：元素在画布中的 X/Y 位置、元素与画布边缘的间距
- 适配**不会**改变：字号大小、图标尺寸、按钮宽高、组件内部间距。这些在设计分辨率坐标系下必须与设计稿完全一致，有差异就是还原问题，必须报告
...
**P2 大小（含字号）** ——此维度不受屏幕适配影响，必须严格比较
  • 组件自身的宽高尺寸是否与设计稿一致（按钮、卡片、面板的宽度和高度）
  • 组件内文字的字号是否正确——字号差异是高优先级问题，即使视觉上只差1-2px也要标出
  • 图标、头像等子元素的宽高是否正确
  • 适配缩放不改变组件自身尺寸和字号，不要因为"可能是适配导致"而忽略大小差异
...
**P5 色彩**
  • 背景色、前景色、强调色，全部使用 #HEX (中文色名) 格式描述
  • 渐变、透明度、叠加模式
